...
def wait_for_network(timeout=60):
    """Wait up to timeout seconds for DNS resolution (WiFi may not be ready at boot)."""
    start = time.time()
    while time.time() - start < timeout:
        try:
            socket.getaddrinfo("neshama.ca", 443)
            return True
...
def main():
    if not wait_for_network():
        print("ERROR: No network after 60s — skipping monitor run")
        return 1
